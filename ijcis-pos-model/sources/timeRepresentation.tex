...
\centerline{\small TIME POINT DATA TYPES}
\vglue-6pt
...
\label{def:open-func}
$Open(C)$\\
Consider an ill-known value $T$, a binary relationship $B_r$ one of $\left(\leq, <, >, \geq\right)$ and the constraint $C\stackrel{\triangle}{=}\left(R,T\right)$.
...
I_p \left(C \right) &=& \Big(\sup_{r\in\T \Rp\  w}\pi_{T}(w)\Big)\\
I_n \left(C \right) &=& \Big(\inf_{r\in\T \Rn\  w}1-\pi_{T}(w)\Big)
\end{eqnarray}
...
\vglue13pt
%\begin{table}[htbp]
\tcap{Relations for the $\Open(C)$ function. Depending on the relation $B_r = \left(\leq, <, >, \geq\right)$ in the constraint $C$, the values for $\Rp$ and $\Rn$ are shown.}
...
In order to properly represent  an ill-known time interval in a database, some datatypes are needed. Because of the ill-known constraints, not all the combinations of datatypes for each ill-known time point (see Table \ref{tbl:time-point-types}) are allowed. Table \ref{tbl:time-interval-types} shows all the possible datatypes combinations for the  for an ill-known time interval denoted by $\left[X, Y\right]$.
