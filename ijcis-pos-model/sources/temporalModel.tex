...
The model is based on GEFRED~\cite{Medina1994} (Generalized Model of Fuzzy Relational DB) model. This model is extended by adding valid-time support which will be illustrated with an example. The information in the system is defined by the following elements:
...
\textbf{Typeof(D).}
The function typeof$\left(D \right)$ returns the datatype associated with the domain $D$. For every datatype, the function returns the associated number as shown in Table \ref{tbl:gefred-data-types}.
\end{definition}
...
\hline
\multirow{3}{*}{$\Body$ } &  3 & Dracula & [15/3/2012, 5, 2] , [30/3/2012, 1, 1] \\
...
%& & & & \centerline{PVP}\\
{$\Head$}  &  &  & &(Start Loan\\
& \textbf{ID}  & \textbf{V} & Title& , End Loan)	\\
 \hline
\multirow{4}{*}{$\Body$}  & 3 & 001 & Dracula & [15/3/2012, 5, 2], [30/3/2012, 1, 1] \\
