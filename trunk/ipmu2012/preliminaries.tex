...
The problem of interval evaluation is more generally explained in \cite{Pon11}: for a crisp interval $I = \left[a, b \right]$, the need exists to know if all points in this interval reside between the boundaries of an ill-known interval $\left[ X , Y \right]$.

The notion of an \emph{ill-known constraint} is introduced:

\begin{definition}
Given a universe $U$, an \emph{ill-known constraint} $C$ on a set $A \subseteq U$ is specified by means of a binary relation $R \subseteq U^{2}$ and a fixed, ill-known value denoted by its variable $X$ over $U$, i.e.:
\begin{align}
C \triangleq (R,X)
\end{align}
\end{definition}
