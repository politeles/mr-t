...
\begin{definition}
\label{def:close-a-crisp-interval}
Consider two crisp intervals $I= \left[a ,b \right]$ and $J= \left[c ,d \right]$, $c > a$. The CloseL$\left(I, J\right)$ function allows to close the left-open interval $I$ with respect to the first value $d$ in $J$:
\begin{align}
...
Analogously, it is defined the function to close an right open interval: 
\begin{definition}
\label{def:close-a-crisp-interval-r}
CloseR$\left(I, J\right)$ 
\begin{align}
...
Consider an object $A$ and a relation $R$ in a temporal database. The fucntion Current$\left(R, A \right)$ obtains the tuple $\left(A, I \right)$ that is current in the relation $R$, that is it, $I = \left[a, +\infty \right]$.
...
Now it is possible to close the current version of an object by combining the two previously defined functions.
...
\text{Close-current} \left(R, A \right) =\\
\begin{cases}
\nonumber
\big \lbrace R - \left(A, I \right) \cup \left \lbrace \left(A, \mbox{ Close } \left(I, J\right) \right) \cup \left(A, J\right)\right \rbrace  \big \rbrace \\
\nonumber
\mbox{ if } I = \mbox{ Current } \left(R, A \right) \wedge J > \mbox{ Close } \left(I, J \right)   \\
...
\item The object $A$ is in the relation $R$, but it is not current 
