...
\nonumber
\forall i, i' \in \left \lbrace 1, \ldots, m\right \rbrace , \exists s \in S: \left(A_{Gs}:d_{is} \right) \neq \left(A_{Gs}:d_{i's} \right)
\end{align}
\end{definition}

\texcolor{red}{primary temporal key}

\begin{definiton}
\label{def:generalized-fuzzy-temporal-key}
A generalized fuzzy temporal key, $K_{GT}$ is a subset od the head:
